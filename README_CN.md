...
DeepSeek Harness 原生外挂插件：**多厂商订阅额度与余额徽章**。支持 **OpenCode（Zen Go）**、**MiniMax**、**Kimi Code** 与 **DeepSeek** 四家厂商，实时查询各厂商的限流额度窗口、订阅配额及账户余额，并在输入框模型选择器旁直观渲染微型额度徽章，支持模型列表动态热同步与完善的 Web 设置卡片。
...
- **🔄 OpenCode 实时模型列表同步**
  - 自动拉取 OpenCode `/models` 最新接口清单，与 DSH 目录合并并写入 `llm-pi-ai` 设置，上游发布新模型后无需重新发布插件即可在选择器中即时可选。
- **🛡️ 服务端代理转发（免 CORS 与密钥安全）**
  - 由 Node.js 服务端定时后台轮询（默认 60s），多开浏览器标签页不增加上游请求，完美绕过浏览器 CORS 跨域限制，且敏感 API Key 绝不出服务端。
- **⚙️ 原生 Web 设置管理卡片**
  - 深度集成于 **设置 → 插件 → 订阅额度徽章**，提供通用设置、厂商动态添加/移除、密钥管理与模型能力控制。
...
│  - OpenCode 实时 /models 同步与 pi-ai 配置写入                │
└──────────────────────────────▲──────────────────────────────┘
                               │ 上游 HTTPS
┌──────────────────────────────┴──────────────────────────────┐
│                    各 AI 厂商官方接口                        │
...
| `GET` | `/api/quota-badges/models` | 获取 OpenCode 实时模型列表与同步状态。 |
...
本插件包含 80 余项单元测试，全面覆盖厂商协议解析、容错降级及模型合并算法：
